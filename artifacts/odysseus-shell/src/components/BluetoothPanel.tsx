    ...
    <div className="mx-auto max-w-3xl space-y-4 p-4" data-testid="bluetooth-panel">
      <Card>
        ...
        <CardContent className="flex items-center justify-between">
          ...
          <Switch checked={!!data?.powered} disabled={!data || powerMut.isPending} onCheckedChange={(v) => powerMut.mutate(v)} data-testid="switch-bluetooth-power" />
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <Button size="sm" variant="outline" className="h-8 gap-1.5" onClick={() => scanMut.mutate()} disabled={!data?.powered || scanMut.isPending} data-testid="button-bluetooth-scan">
            ...
            devices.map((d) => (
              <div key={d.mac} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm" data-testid={`bt-device-${d.mac}`}>
                ...
                <div className="flex items-center gap-1">
                  {!d.paired && (
                    <Button size="sm" variant="secondary" className="h-7 gap-1" disabled={actionMut.isPending} onClick={() => actionMut.mutate({ action: "pair", mac: d.mac })} data-testid={`button-bt-pair-${d.mac}`}>Pair</Button>
                  )}
                  <Button size="sm" variant="ghost" className="h-7 gap-1" disabled={actionMut.isPending} onClick={() => actionMut.mutate({ action: "connect", mac: d.mac })} data-testid={`button-bt-connect-${d.mac}`}>
                    <Link className="h-3.5 w-3.5" /> Connect
                  </Button>
                  <Button size="sm" variant="ghost" className="h-7 gap-1" disabled={actionMut.isPending} onClick={() => actionMut.mutate({ action: "remove", mac: d.mac })} data-testid={`button-bt-remove-${d.mac}`}>
