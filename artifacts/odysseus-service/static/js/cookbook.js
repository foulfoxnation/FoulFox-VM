  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside FoulFox OS.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('FoulFox OS app', 'Run inside the FoulFox OS app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend FoulFox OS capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install FoulFox OS keys, choose model directories, and set the default server. Local is this machine.</p>';
